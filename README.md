...
**Backend** (из корня репозитория, с активным Python-виртуальным окружением):
```bash
pytest
```
Покрывает ветки отказа fake-провайдера, эндпоинты `/v1/briefs/decode` и `/v1/briefs/runs/{id}`, логику Pydantic-валидации и классификации ошибок. Требует сетевого доступа к тому, на что указывает `DATABASE_URL` (Postgres) - запускайте против доступной базы данных, например через `docker compose up -d postgres` или внутри контейнера `api`.
