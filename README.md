...
│   ├── services/decode_service.py# оркестрация (провайдер -> валидация -> сохранение) + сборка API-ответа
│   ├── providers/                # LLMProvider protocol + FakeProvider + RealProvider
│   ├── repositories/             # доступ к БД для decode runs (только ORM, не знает о форме API-ответа)
...
- Проверить `RealProvider` на реальных ответах модели за пределами happy path (юнит-тесты на конфигурацию есть, но не на разнообразие реальных ответов Gemini).
